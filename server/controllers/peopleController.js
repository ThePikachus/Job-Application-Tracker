...
const instructors = [
  ...
  {name: "Nathan Gonsalves", img:''},
  {name: "Rodrigo Calderon", img:''},
  {name: "Sean Kelly", img:''}
];
...
pool.query(createResidentTableQuery, (error, results) => {
  ...
  for(const key in residents){
    ...
    pool.query(insertResidentDataQuery, values, (error, results) => {
      if (error) throw error;

      if (results.rowCount > 0) {
        console.log('Data inserted successfully.');
      } else {
        console.log('Data already exists. Skipped insertion.');
      }
...
pool.query(createInstructorTableQuery, (error, results) => {
  if (error) throw error;
  console.log('Table created successfully.');
  ...
  for(const key in instructors){
    const values = [key.name, "", [], key.img, "", "codesmith", "white"]
    // Insert data only if it doesn't already exist
    pool.query(insertInstructorDataQuery, values, (error, results) => {
      if (error) throw error;
